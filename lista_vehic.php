 ...
 session_start();  
  if ($_SESSION["nivel"]=='3') {
   ...
   include("php/func_adminterminal.php");
	 $query = "SELECT * from linea where estado='activa' order by nombre_linea";
	 $result = mysql_query($query);
	 	 $opciones = "<option value=' '>Elegir</option>";
	  while ($row = mysql_fetch_assoc($result)){
	$opciones.='<option value="'.$row['cod_linea'].'">'.htmlentities($row['nombre_linea']).' ('.htmlentities($row['ruta']).')</option>';
}  
	 switch(date("m")){
	 case 1: 
	 $mes="Enero";
...
<div >
 ...
 <form  method="post" enctype="multipart/form-data" name="form1" id="form1" onsubmit="return false" ><fieldset >
  ...
  <table width="820" border="0" cellspacing="0">
    <tr>
      <td width="30%" align="right">Nombre de la línea&nbsp;&nbsp;&nbsp;&nbsp; </td>
      <td width="70%" align="left"><label for="ced_usu"></label>
        <label for="linea"></label>
        <select name="linea" class="Estilo14" id="linea" onchange="xajax_lista_vehi(xajax.getFormValues('form1'))">
          <? echo $opciones;?>
        </select></td>
    </tr>
    <tr>
